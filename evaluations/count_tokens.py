...
def load_tokenizer(model_checkpoint : str='gpt2-xl') -> AutoTokenizer:
    """Loads a tokenizer from specified checkpoint"""
    return AutoTokenizer.from_pretrained(model_checkpoint)

def count_tokens(tokenizer : AutoTokenizer, texts : List[str]) -> List[int]:
    """For a given set of texts and tokenizer, counts how many tokens are generated by the tokenizer for each text"""
    return [len(tokenizer.encode(text)) for text in texts]

def compute_token_statistics_countermoral(datasets : dict, tokenizer : AutoTokenizer, aggregate_frameworks_seperate : bool=True) -> dict:
    """Computes sample statistics of the token counts of the action, target_new and target_true fields across different portions of CounterMoral

    :param datasets (dict): Contains the entire countermoral dataset, organized by framwork and whether it is the fine-grained or course-grain portion
    :param tokenizer: Tokenizer for generating tokens
    :param aggregate_frameworks_seperate (bool): Determines whether to evaluate the ethical frameworksseperately or all together.

    return: A dictionary containing the sample statistics
    """
...
def plot_results(analysis : dict) -> None:
    """Plots the sample statistics computed of the token counts"""
...
def plot_comparison_results(c_moral_stats : dict, c_fact_stats : dict) -> None:
    """Compares results between CounterFact and CounterMoral token counts of the actions, and objects"""
