...
def evaluate_entry(data_entry : dict, model_type : str, hparams, edit_technique : str) -> dict:
    """Uses EasyEdit API to edit the base model with the provided edit template, and then evaluates the edit and returns the resulting metrics"""
    ...
    # Compute the neighbourhood score, by taking the difference in pre/post edit scores on out-of-distribution prompts
...
def evaluate_entries_batch(dataset : List[dict], model_type : str, hparams, edit_technique : str) -> List[dict]:
    """Analagous to `evaluate_entries()`, except that it edits the model on a batch collection of entries before evaluating (used for MEMIT). Note that this is not analagous to just running `evaluate_entries()` repeatedly, as the base model gets edited on all examples before evaluations are run."""
    ...
    # Edit the base model on the entire dataset
    metrics, edited_model, _ = editor.edit(
            prompts=prompts,
			ground_truth=target_true,
			target_new=target_new,
...
def run_evaluations(model : str, edit_technique : str, ethical_framework : str, actions_broad : bool, model_type : str):
    """Main function which both runs the evaluations and writes the results"""
